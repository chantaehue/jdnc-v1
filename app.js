...
function updateAISolutionPage(analysis) {
    try {
        console.log('📊 updateAISolutionPage 호출됨:', analysis);

        // [FIX] 대시보드 페이지에서 결과를 표시
        const dashboardPage = document.querySelector('#dashboard-page');
        if (!dashboardPage) {
            alert('❌ dashboard-page를 찾을 수 없음');
            console.error('❌ dashboard-page를 찾을 수 없습니다');
            return;
        }

        // 양액 분석 결과 카드 찾기
        let resultCard = document.getElementById('nutrient-solution-card');

        if (!resultCard) {
            alert('❌ nutrient-solution-card를 찾을 수 없음');
            return;
        }
        ...
        if (statusBadge) {
            statusBadge.className = `status-badge ${analysis.statusClass}`;
            statusBadge.textContent = `작물 상태: ${analysis.status}`;
        }
        ...
        if (targetInfo) {
            let summaryText = `VPD ${analysis.vpd} kPa | 온도차 ${analysis.tempDiff}°C`;
            if (analysis.ecDiff) {
                summaryText += ` | EC 변화 ${analysis.ecDiff} | pH 변동 ${analysis.phTrend}`;
            }
            targetInfo.textContent = summaryText;
        }
        ...
        console.error('updateAISolutionPage 오류:', error);
    }
}
    ...
    form.addEventListener('submit', (e) => {
        ...
        try {
            ...
            const getCropName = () => {
                if (isPremiumActive) {
                    const el = document.getElementById('nutrient-crop-select');
                    return el && el.selectedIndex >= 0 ? el.options[el.selectedIndex].text : '딸기';
                } else {
                    const el = document.getElementById('select-crop');
                    return el && el.selectedIndex >= 0 ? el.options[el.selectedIndex].text : '딸기';
                }
    ...
    if (typeof db !== 'undefined' && db) {
        db.collection('settings').doc('notice').get().then((doc) => {
            if (doc.exists) {
                ...
                // [FIX] Firestore에 공지사항이 없으면 localStorage도 삭제하고 숨김 (관리자 권한 존중)
                localStorage.removeItem('smartfarm_notice');
                noticeEl.classList.add('hidden');
                console.log('☁️ Firestore에 공지사항 없음 (동기화: 데이터 삭제)');
            }
        }).catch(e => {
